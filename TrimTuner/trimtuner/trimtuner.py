...
#bootstrapping
from trimtuner.trimtuner.initial_sampling import initial_sampling
...
class logs():
    ...
    def __init__(self, seed, initSamples, model, heuristic):
        ...
        while os.path.isfile(filename):
            filename = filename_orig + "_" + str(counter)
            counter += 1
            if counter >= 10000:
    ...
    def printLogs(self, it, inc, incTime, incAcc, incCost, conf, confTime, confAcc, confCost, overhead, CumulativeCost):
        ...
        strWrite = str(self.seed) + ";" + str(self.initSamples) + ";" + str(it) + ";" + inc + ";" + str(incTime) + ";" + str(incAcc) + ";" + str(incCost) + ";" + conf + ";" + str(confTime) + ";" + str(confAcc) + ";" + str(confCost) + ";" + str(overhead) + ";" + str(CumulativeCost) + "\n"
...
def trimtuner(objective_function, all_configs, constraints, seed, filterHeuristic, model,
            lower, upper, s_min, s_max, n_init=30, num_iterations=100, subsets=[256, 128, 64]):
    ...
    if model == "dt":
        ...
        number_trees = 10
        model_objective = EnsembleGPs(number_trees)
        model_cost = EnsembleGPs(number_trees)
    ...
    x_init = initial_sampling(seed, unexplored_Set, real_n_init)
    ...
    for it in range(X.shape[0]+1, num_iterations+1):
        ...
        print("Evaluate candidate %s on subset size %f" % new_x, int(s))
        sys.stdout.flush()
        ...
        inc, inc_acc, inc_cost, inc_time = incumbent_estimation_cea(model_objective, model_cost, X[:, :-1], constraints, )
        inc[-1] = retransform(inc[-1], s_min, s_max)

        print("Current incumbent %s with estimated accuracy of %.3f %" % (inc, inc_acc))
        ...
        print("Optimization overhead was %f seconds" % (total_overhead))
        sys.stdout.flush()

        #write logs in the files
        logs.printLogs(counter_it, inc, inc_time, inc_acc, inc_cost, tested_config, new_t, 1-new_y, new_c, total_overhead, costCumulative)


    logs.close()

    results = "\n The optimal configuration is " + inc + " with estimated accuracy of " + str(inc_acc) + "\0025 and a cost of " + str(inc_cost) + "\n"
    print(results)

    print("\n The optimal configuration is %s with estimated accuracy of %.3f\u0025 and a cost of %.3f\n" % (inc, inc_acc, inc_cost))
